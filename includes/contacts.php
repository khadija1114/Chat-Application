<?php
	...
	<div style ="text-align: center; anidmation: appear 0.75s ease"> ';
		...
		foreach ($myusers as $row){
			...
			<div id = 'contact' style ='position: relative;' userid='$row->userid' onclick = 'start_chat(event)'>
				...
				if (count($msgs) > 0 && isset($msgs[$row->userid])) {
					$mydata .= "<div style ='width: 20px; height: 20px; border-radius: 50%; background-color: orange; color: white; position: absolute; left:-5px; top:-5px;'>".$msgs[$row->userid]."</div>";
				}
